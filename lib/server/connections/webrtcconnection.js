...
class WebRtcConnection extends Connection {
  constructor(id, options = {}) {
    ...
    const { RTCPeerConnection, beforeOffer, timeToConnected, timeToReconnected } = options;
    ...
    peerConnection.addEventListener("icecandidate", ({ candidate }) => {
      self._gatheredCandidates.push(candidate);
    });
    ...
    this.doOffer = async () => {
      ...
      try {
        // ice candidate doesn't usually happen before peers apply remote descriptions
    ...
    this.applyAnswer = async answer => {
      ...
      this._iceCandidates = await waitUntilIceGatheringStateComplete(peerConnection, options);
    };
    ...
    this.toJSON = () => {
      return {
        ...
        gatheredCandidates: this.gatheredCandidates
      };
    };
    ...
    Object.defineProperties(this, {
      gatheredCandidates: {
        get() {
          return this._gatheredCandidates;
        }
      },
      iceConnectionState: {
        get() {
...
async function waitUntilIceGatheringStateComplete(peerConnection, options) {
  ...
  function onIceCandidate({ candidate }) {
    if (!candidate) {
      ...
      //      peerConnection.gatheredCandidates.push(candidate);
    }
  }
