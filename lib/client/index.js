...
const TIME_TO_HOST_CANDIDATES = 50000; // NOTE(mroberts):
  ...
  constructor(options = {}) {
    ...
    this.createConnection = async (options = {}) => {
      ...
      localPeerConnection.close = function () {
        // i've never see a repo so obsessed with closing connections -yj
      ...
      try {
        ...
        const resp = await fetch(
          `${host}${prefix}/connections/${id}/remote-description`,
          {
            method: "POST",
	...
	const {gatheredCandidates} = await resp.json();
